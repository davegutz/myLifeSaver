...
def list_stochastic_run_ids(csv_path: str) -> list[int]:
    """Return the sorted list of integer run_ids from the LHS Gutz CSV."""
...
def prompt_for_run_id(csv_path: str) -> int:
    ...
    try:
        ...
        raise FileNotFoundError(
            f"LHS Gutz CSV not found: '{csv_path}'. "
            f"Run Run_LHS_Gutz_Taylor.py first or supply --lhs-csv."
        )
...
def load_scenario_from_csv(csv_path: str, run_id: int) -> tuple[LhsScenario, dict]:
    """
    Read a stochastic LHS Gutz row from the CSV and reconstruct its
    LhsScenario and context.  Returns (scenario, context_dict).
    """
    df = pd.read_csv(csv_path)
    df["run_id"] = df["run_id"].astype(str)
